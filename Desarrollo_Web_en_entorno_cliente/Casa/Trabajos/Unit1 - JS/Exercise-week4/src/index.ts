...
restaurants.getAll().then((e) =>
    e.restaurants.forEach((element: Restaurant) => {
        showRestaurants(element);
    })
);

async function deleteRestaurant(element: Restaurant, divCol: HTMLElement) {
...
function showRestaurants(element: Restaurant) {
    ...
    const producHTML = RESTAURANT({
        ...
        daysOpen: days(element.daysOpen),
        danger: (openOrClose === "Open" ? "bg-success" : "bg-danger"),
        openClosed: openOrClose,
        phone: element.phone, cuisine: element.cuisine
    });
